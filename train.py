...
def _train(cfg: DictConfig):
    assert wandb.login(), "wandb not logged in"
    ckpt_ft = None
    if cfg.resume is not None:
        logger.info(f'Resume training: {cfg.resume}')
...
def train(cfg: DictConfig, ckpt_ft: Optional[str] = None) -> None:
    ...
    list_of_all_feats = data_module.nfeats[:5]
    cfg.model.input_feats = cfg.data.load_feats[:5]
